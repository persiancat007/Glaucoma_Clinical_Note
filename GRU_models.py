...
class Classification_RNN(nn.Module):
    def __init__(self, num_layers,hidden_dim, embedding_dim, batch_size, vocab_size_note, vocab_size_gpt4, dropout=0.2):
        ...
        self.gru_note = torch.nn.GRU(embedding_dim, self.dim, self.n_layers, batch_first=True)  # Use torch.nn.GRU
        self.gru_gpt4 = torch.nn.GRU(embedding_dim, self.dim, self.n_layers, batch_first=True)  # Use torch.nn.GRU
    ...
    def forward(self, x,hidden):
        ...
        gru_out_note, hidden_note = self.gru_note(embeds_note, hidden)    
        gru_out_gpt4, hidden_gpt_4 = self.gru_gpt4(embeds_gpt_4, hidden)
        ...
        # lstm_out_note = lstm_out_note.contiguous().view(-1, self.hidden_dim)        # Kaggle Sentiment Analysis using LSTM PyTorch.  uncomment this line
        # lstm_out_gpt4 = lstm_out_gpt4.contiguous().view(-1, self.hidden_dim)        # Kaggle Sentiment Analysis using LSTM PyTorch. uncomment this line
        ...
        # out = self.fc(self.ReLU(cat_hidden_gpt_4_notes[:,-1]))
